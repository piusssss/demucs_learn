...
class FlipChannels(nn.Module):
    ...
    def forward(self, wav):
        ...
        if self.training and wav.size(2) == 2:
            if sources == 2:
                left = th.randint(2, (batch, 1, 1, 1), device=wav.device).expand(-1, sources, -1, -1)
...
class FlipSign(nn.Module):
    ...
    def forward(self, wav):
        ...
        if self.training:
            if sources == 2:
                signs = th.randint(2, (batch, 1, 1, 1), device=wav.device, dtype=th.float32).expand(-1, sources, -1, -1)
                wav = wav * (2 * signs - 1)
            else:
...
class Scale(nn.Module):
    ...
    def forward(self, wav):
        ...
        if self.training and random.random() < self.proba:
            if streams == 2:
                scales = th.empty(batch, 1, 1, 1, device=device).uniform_(self.min, self.max).expand(-1, streams, -1, -1)
                wav *= scales
            else:

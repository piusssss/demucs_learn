...
def repitch(wav, pitch, tempo, voice=False, quick=False, samplerate=44100):
    ...
    infile_path = os.path.join(temp_dir, f"demucs_repitch_in_{uuid.uuid4().hex}.wav")
    outfile_path = os.path.join(temp_dir, f"demucs_repitch_out_{uuid.uuid4().hex}.wav")
    ...
    infile = TempFile(infile_path)
    outfile = TempFile(outfile_path)
    save_audio(wav, infile.name, samplerate, clip='clamp')
    
    soundstretch_path = "soundstretch"
    ...
    try:
        ...
        infile.close()
        outfile.close()
        raise RuntimeError(f"Could not change bmp because {error.stderr.decode('utf-8')}")

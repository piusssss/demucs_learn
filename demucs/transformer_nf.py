...
class CrossTransformerEncoder(nn.Module):
    def __init__(
        ...
        # Input normalization for multi-resolution features
        ...
        # Multi-resolution self-attention layers
        ...
        kwargs_encoder = dict(kwargs_common)
        kwargs_encoder.update({
            "sparse": sparse_self_attn,
        })

        # Create self-attention layers for multi-resolution interaction
        for idx in range(num_layers):
            self.layers.append(MyTransformerEncoderLayer(**kwargs_encoder))
    ...
    def forward(self, x_list):
        ...
        # Then: Add positional embeddings in the square space
        grouped_list = []
        ...
        # Multi-scale positional encoding in unified square space:
        # 1. 2D position in square [num_res=4, tokens_per_group=32]
        # 2. Resolution embedding (learnable, for flexibility)
        # 3. Time group position [min_time_steps=54]
        
        # Add 2D positional embedding on the square structure
        # Row dimension (num_res=4): which resolution (0-3)
        # Col dimension (tokens_per_group=32): position within resolution (0-31)
        pos_emb_2d_square = create_2d_sin_embedding(
            C, self.num_resolutions, tokens_per_group, device, self.max_period
        )  # [1, C, 4, 32]
        
        # Expand to all time groups: [B, C, 54, 4, 32]
        pos_emb_2d_square = pos_emb_2d_square.unsqueeze(2)  # [1, C, 1, 4, 32]
        pos_emb_2d_square = pos_emb_2d_square.expand(B, C, min_time_steps, self.num_resolutions, tokens_per_group)
        all_res = all_res + self.weight_pos_embed * pos_emb_2d_square
        
        # Add resolution embedding (learnable, provides flexibility beyond fixed 2D encoding)

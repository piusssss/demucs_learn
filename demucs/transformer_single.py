...
class CrossTransformerEncoder(nn.Module):
    def __init__(
        ...
        self.cross_first = cross_first  # Store for use in forward
        self.freq = freq  # Store which branch to process
        self.emb = emb
        ...
        # Single set of layers (not separate for freq and time)
        ...
        # Create single set of layers based on cross_first
    ...
    def forward(self, x, xt):
        if self.freq:
            # Only process x branch (frequency domain)
            B, C, Fr, T1 = x.shape
            ...
            # Process xt if needed for cross-attention
            if self.cross_first:
                if xt.dim() == 4:
                    # xt is also frequency domain (4D)
                    B_t, C_t, Fr_t, T1_t = xt.shape
                    pos_emb_2d_t = create_2d_sin_embedding(
                        C_t, Fr_t, T1_t, xt.device, self.max_period
                    )
                    pos_emb_2d_t = rearrange(pos_emb_2d_t, "b c fr t1 -> b (t1 fr) c")
                    xt = rearrange(xt, "b c fr t1 -> b (t1 fr) c")
                    xt = self.norm_in_t(xt)
                    xt = xt + self.weight_pos_embed * pos_emb_2d_t
                else:
                    # xt is time domain (3D) - follow original logic exactly
                    B_t, C_t, T2 = xt.shape
                    assert C_t % 2 == 0, f"C_t must be even, got {C_t}, xt.shape={xt.shape}"
                    xt = rearrange(xt, "b c t2 -> b t2 c")  # Now shape is (B, T2, C)
                    pos_emb = self._get_pos_embedding(T2, B_t, C_t, x.device)  # C_t is the channel dim
            ...
            # Rearrange xt back if it was processed for cross-attention
            if self.cross_first:
                if xt.dim() == 3 and xt.shape[1] != C:  # xt was rearranged to (B, T, C) or (B, T*Fr, C)
                    # Need to rearrange back
                    if len(xt.shape) == 3 and xt.shape[2] == C:
                        # Time domain: (B, T2, C) -> (B, C, T2)
                        xt = rearrange(xt, "b t2 c -> b c t2")
            
            return x, xt
        else:
            # Only process xt branch (time domain)
            B, C, T2 = xt.shape
            xt = rearrange(xt, "b c t2 -> b t2 c")
            pos_emb = self._get_pos_embedding(T2, B, C, xt.device)
            pos_emb = rearrange(pos_emb, "t2 b c -> b t2 c")
            xt = self.norm_in_t(xt)
            xt = xt + self.weight_pos_embed * pos_emb

...
class TensorChunk:
    ...
    def padded(self, target_length):
        ...
        pad_left = correct_start - start  #max-offset
        pad_right = end - correct_end     #max+offset 

        out = F.pad(self.tensor[..., correct_start:correct_end], (pad_left, pad_right))
        assert out.shape[-1] == target_length     #我认为冗余
        return out
...
def apply_model(model: tp.Union[BagOfModels, Model],
    ...
    if shifts:
        ...
        scale = float(format(stride / model.samplerate, ".2f"))  #每处理一个分块，跨越xx秒音频

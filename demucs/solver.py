    ...
    def _reset(self):
        ...
        if self.checkpoint_file.exists():
            ...
            self.model.load_state_dict(package['state'])
             # 修复：只有在continue_opt为True时才加载优化器状态
            if getattr(self.args, 'continue_opt', True):  # 默认为True以保持向后兼容
                self.optimizer.load_state_dict(package['optimizer'])
            else:
                logger.info('Skipping optimizer state loading (continue_opt=False)')
    ...
    def _run_one_epoch(self, epoch, train=True):
        ...
        for idx, sources in enumerate(logprog):
            ...
            if train:
                ...
                if sources.shape[1] == 2:  # 2个源：mixture + instrumental
                    mix = sources[:, 0]    # 使用mixture作为输入
                    sources = sources[:, 1:]  # instrumental作为目标
                else:
                    
                    mix = sources.sum(dim=1)  # 混合所有源
            else:
                mix = sources[:, 0]
                sources = sources[:, 1:]

...
class CrossTransformerEncoder(nn.Module):
    ...
    def forward(self, x_list, xt):
        # x_list: list of [B, C, Fr_i, T_i]
        shapes = [(x.shape[2], x.shape[3]) for x in x_list]
        B, C = x_list[0].shape[0], x_list[0].shape[1]
        device = x_list[0].device
        
        # Encode each resolution with 2D pos + resolution embedding
        x_encoded = []
        for i, x in enumerate(x_list):
            Fr, T1 = x.shape[2], x.shape[3]
            pos_emb_2d = create_2d_sin_embedding(
                C, Fr, T1, device, self.max_period
            )  # (1, C, Fr, T1)
            pos_emb_2d = rearrange(pos_emb_2d, "b c fr t1 -> b (t1 fr) c")
            x = rearrange(x, "b c fr t1 -> b (t1 fr) c")
            x = self.norm_in(x)
            x = x + self.weight_pos_embed * pos_emb_2d
            res_emb = self.resolution_embedding(torch.tensor([i], device=device))
            x = x + res_emb.unsqueeze(1)
            x_encoded.append(x)
        
        x = torch.cat(x_encoded, dim=1)
        ...
        x_splits = torch.split(x, lengths, dim=1)
        x_list_out = []
        for x_flat, (Fr, T1) in zip(x_splits, shapes):
            x_out = rearrange(x_flat, "b (t1 fr) c -> b c fr t1", t1=T1, fr=Fr)
            x_list_out.append(x_out)
        
        xt = rearrange(xt, "b t2 c -> b c t2")
        return x_list_out, xt

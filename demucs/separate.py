...
def main(opts=None):
    # 硬编码调试参数
    #opts = ['-n', 'htdemucs_ft', '--shifts=2', '--two-stems', 'vocals', '--overlap', '0.25', 'bilibili/war.wav']
    parser = get_parser()
    ...
    try:
        separator = Separator(model=args.sig if args.sig else args.name,
                              repo=args.repo,
                              device=args.device,
                              shifts=args.shifts,
                              split=args.split,
    ...
    out = args.out / (args.sig if args.sig else args.name)

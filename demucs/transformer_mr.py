...
class CrossTransformerEncoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        """
        Single-branch self-attention Transformer for MR architecture
        """
        ...
        # Input normalization (only for frequency branch now)
        ...
        # Create only self-attention layers (no cross-attention needed)
        for idx in range(num_layers):
            self.layers.append(MyTransformerEncoderLayer(**kwargs_common))

    def forward(self, x):
        """
        Single input self-attention only (no cross-attention with time branch)
        Args:
            x: [B, C, Fr, T] frequency domain features
        Returns:
            x: [B, C, Fr, T] transformed features
        """
        B, C, Fr, T = x.shape
        
        # 2D positional embedding for frequency-time
        pos_emb_2d = create_2d_sin_embedding(
            C, Fr, T, x.device, self.max_period
        )  # (1, C, Fr, T)
        pos_emb_2d = rearrange(pos_emb_2d, "b c fr t -> b (t fr) c")
        
        # Flatten spatial dimensions
        x = rearrange(x, "b c fr t -> b (t fr) c")
        x = self.norm_in(x)
        x = x + self.weight_pos_embed * pos_emb_2d

        # Apply self-attention layers only
        for idx in range(self.num_layers):
            # All layers are self-attention now
            x = self.layers[idx](x)

        # Reshape back to spatial format
        x = rearrange(x, "b (t fr) c -> b c fr t", t=T, fr=Fr)
        return x
